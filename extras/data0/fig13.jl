...
@everywhere function simulate_sample(rng::AbstractRNG, N, l, γc)
    P = LinearAlgebra.cholesky(I(8))
    k = [0.625 1.498 1.162; 0.819 1.806 1.359; 1.051 1.994 1.652]
    ỹ = [P.L * Random.randn(rng, 8) for i ∈ 1:N]
    x = [1 + (i - 1) % 4 for i ∈ 1:N]
    γ = γvec(4, γc)
    for i in 1:N
        zi = rand(rng, [-1, 1])
...
CSV.write("data/fig13.csv", df, quotestrings = true)